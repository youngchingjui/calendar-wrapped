...
export function calculateStats(events: any[]) {
  ...
  events.forEach((event) => {
    // Count locations
    if (event.location) {
      stats.locations[event.location] =
        (stats.locations[event.location] || 0) + 1
    }
    ...
    if (
      event.attendees &&
      event.attendees.length > stats.mostAttendeesEvent.attendees.length
    ) {
    ...
    if (event.attendees) {
      event.attendees.forEach((attendee) => {
        stats.attendeesCount[attendee.email] =
          (stats.attendeesCount[attendee.email] || 0) + 1
      })
    }
  })
  ...
  const busiestMonth = new Date(2024, busiestMonthIndex).toLocaleString(
    "default",
    { month: "long" }
  )
  ...
  let busiestDay = busiestDayDate?.toLocaleDateString("default", {
    year: "numeric",
    month: "long",
    day: "numeric",
  })
  ...
  const mostPopularOnlineMeetingPlatform = Object.entries(
    stats.locations
  ).reduce((a, b) => (b[1] > a[1] ? b : a), ["", 0])[0]

  // Find the top attendees
  stats.topAttendees = Object.entries(stats.attendeesCount)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .map(([email, count]) => ({ email, name: email, count }))
